...
( function ( $ ){
  ...
  x.controller = {
    ...
    _idGEN : function () {
      //id generated by date + milliseconds every time an widgets is requested
      var d = new Date();
    ...
    _loadWidget : function ( widget, location ) {
      x.controller._loadCSS(location+".css");   // load CSS for each widget
      require( [ 'text!'+location+'.html', location ], function ( html ) {  // get HTML for widget

        $("xtyle[type=widget][name="+widget+"]").each(function(){  // for each occurrence 

          var id = "#"+$(this).attr("id");
          $(id).html(html)[ widget ]();
          //$(id+"> div").eq(0).attr("id", "mariz");
          //console.log($(id).html());
          //console.log($(original));
          //.find(":first-child").attr("id", id) [ widget ] ();

        });// each widget with same class on page

        //$("xtyle[type=widget] ."+widget).replaceWith(html);
      });
